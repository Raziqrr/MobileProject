 ...
 * @Date: 2025-05-08 17:29:35
 * @LastEditors: Raziqrr rzqrdzn03@gmail.com
 * @LastEditTime: 2025-05-09 11:12:37
 * @FilePath: app/src/main/java/com/example/mobileproject/screens/ai_assisstant/ChatViewModel.kt
...
"""
    ...
    fun onInputChanged(newInput: String) {
        ...
        println("new value")
        println(_uiState.value)
        println(_uiState.value.currentInput)
    ...
    fun sendMessage() {
        ...
        viewModelScope.launch {
            try {
                ...
                val response = generativeModel.generateContent(*conversationHistory.toTypedArray())
                val reply = response.text ?: "Hmm, I didn't get that, mate!"
                conversationHistory.add(content(role = "model") { text(reply) })

                val messageChunks = reply.split("\n\n").map { it.trim() }.filter { it.isNotEmpty() }
                val botMessages = messageChunks.map { chunk ->
                    ChatMessage(message = chunk, isFromUser = false)
                }
                ...
                val errorMsg =
                    ChatMessage(message = "⚠️ Error: ${e.localizedMessage}", isFromUser = false)
                _uiState.value = _uiState.value.copy(
                    chatHistory = _uiState.value.chatHistory + errorMsg,
                    isTyping = false
                )
            }
        }
